...
export class CartPage implements OnInit {
    ...
    ngOnInit() {
      ...
      if(this.values.paymentMethods && this.values.paymentMethods.length) {
        ...
        if(method) {
          ...
          if(this.values.order.payment_method === 'stripe')
          setTimeout(() => {
             this.setupStripe();
          }, 400);
        }
  	...
  	addProduct(index) {
  		this.values.order.line_items[index].quantity++;
    }
    removeProduct(index) {
        let qty = this.values.order.line_items[index].quantity;
		if(qty == 1 || qty == 0) {
    ...
    async editCustomer() {
      const modal = await this.modalCtrl.create({
          ...
          componentProps: {
              billing: this.values.order.billing,
              shipping: this.values.order.shipping,
      ...
      if(data.billing) {
        this.values.order.billing = data.billing;
        this.values.order.shipping = data.shipping;
    ...
    addressAvailable() {
      if(this.values.order.shipping.first_name && this.values.order.shipping.last_name && this.values.order.shipping.address_1 && this.values.order.shipping.city && this.values.order.shipping.postcode && this.values.order.billing.email){
        return true;
      }
      else return false;
    }
    createOrder(){
      if(this.values.order.payment_method === 'cod') {
        this.values.order.status = 'processing';
      }
      this.presentLoading();
      this.values.order.payment_method_title = this.values.paymentMethods.find((item) => item.id === this.values.order.payment_method).title;
      this.service.wcpost('/orders', this.values.order).then((results) => {
        this.order = results;
        this.processPayment();
      }).catch(() => {
    ...
    handlePayPalPayment() {
      this.service.postItem('process_payment', {id: this.order.id}).then(res => {
          ...
          var pageContent = '<html><head></head><body><form id="loginForm" action="' + this.config.url + '/checkout/order-pay/' + this.order.id +'/?pay_for_order=true&key='+ this.order.order_key +'" method="post">' +
    ...
    orderSummary() {
      this.stopLoading();
      this.values.order.line_items = [];
      delete this.values.order.status;
    ...
    setupStripe() {
        ...
        form.addEventListener('submit', event => {
            ...
            var ownerInfo = {
                owner: {
                    name: this.values.order.shipping.first_name + ' ' + this.values.order.shipping.last_name,
                    address: {
                        line1: this.values.order.shipping.address_1,
                        city: this.values.order.shipping.city,
                        postal_code: this.values.order.shipping.postcode,
                        country: this.values.order.shipping.country,
                    },
                    email: this.values.order.billing.email
                },
            };
            if (!this.values.order.shipping) {
                this.values.order.shipping.first_name = this.values.order.billing.first_name;
                this.values.order.shipping.last_name = this.values.order.billing.last_name;
                this.values.order.shipping.company = this.values.order.billing.company;
                this.values.order.shipping.address_1 = this.values.order.billing.address_1;
                this.values.order.shipping.address_2 = this.values.order.billing.address_2;
                this.values.order.shipping.city = this.values.order.billing.city;
                this.values.order.shipping.country = this.values.order.billing.country;
                this.values.order.shipping.state = this.values.order.billing.state;
                this.values.order.shipping.postcode = this.values.order.billing.postcode;
            }
    ...
    getDescription() {
      if(this.values.order.payment_method)
      return this.values.paymentMethods.find((item) => item.id === this.values.order.payment_method).description;
      else return null;
    }
}
