...
export class EditAddressPage implements OnInit {
  ...
  constructor(public service: Service, public settings: Settings, public navCtrl: NavController) { }
   	address: any = [];
    countries: any;
    states: any;
    billingStates: any;
...
async getCountries() {
    await this.service.postItem('countries').then(res => {
        this.countries = res;
...
processAddress() {
    ...
    async updateAddress() {
        ...
        await this.service.postItem('update-address', this.address).then(res => {
            this.status = res;
            this.navCtrl.pop();
