...
export class LoginPage implements OnInit {
    ...
    async onSubmit() {
        ...
        await this.service.postItem('login', this.form.value).then(res => {
            this.status = res;
   ...
   /* googleLogin(){
        ...
        .then(res => {
            ...
            this.service.postItem('google_login', {
    ...
    facebookLogin(){
        ...
        .then((res: FacebookLoginResponse) => {
            ...
            this.presentLoading();
            this.service.postItem('facebook_login', {
    ...
    handlePhoneLogin(info){
        if(info.phoneNumber) {
            this.service.postItem('phone_number_login', {
    ...
    async onRegister() {
        this.disableSubmit = true;
        await this.service.postItem('create-user', this.formRegister.value).then(res => {
            this.status = res;
            if (this.status.errors) {
                ...
                this.settings.customer.id = this.status.ID;
                 if (this.platform.is('cordova'))
                    this.oneSignal.getIds().then((data: any) => {
                        ...
                        this.service.postItem('update_user_notification', this.pushForm).then(res =>{});
                    });
                this.close(true);
                this.disableSubmit = false;
            }
